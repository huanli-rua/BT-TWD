...
for dataset in dataset_dirs:
    ...
    # ⭐ 只筛选 n_test > 0 的桶（All tested leaf buckets）
    tested_buckets = df[df['n_test'] > 0].copy()

    sbsr_dict = {'Dataset': dataset, 'Num_Tested_Buckets': len(tested_buckets)}

    # 计算比例指标 SBSR
    for metric, higher_is_better in metrics_proportion.items():
        baseline_col = baseline_cols[metric]
        mask = tested_buckets[[metric, baseline_col]].notna().all(axis=1) & \
               np.isfinite(tested_buckets[[metric, baseline_col]]).all(axis=1)

        valid = tested_buckets[mask]

        if len(valid) == 0:
            sbsr_ratio = np.nan
        else:
            if higher_is_better:
                sbsr_ratio = (valid[metric] >= valid[baseline_col]).mean()
            else:
                sbsr_ratio = (valid[metric] <= valid[baseline_col]).mean()
    ...
    for metric, higher_is_better in metrics_gain.items():
        ...
        if len(valid) == 0:
            ...
            if higher_is_better:
                diff = valid[metric] - valid[baseline_col]
            else:
                diff = valid[baseline_col] - valid[metric]

            avg_diff = diff.mean()
            weighted_avg_diff = (diff * valid['n_test']).sum() / valid['n_test'].sum()
...
print("8个数据集 All tested buckets (n_test>0) 指标汇总：")
print(sbsr_df)
...
output_file = output_dir / 'sbsr_summary_tested_bucket_proportion_gain.csv'
